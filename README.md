
---
# 0-to-Projects

A collection of beginner-friendly projects and learning resources I found online for students looking to gain practical experience, build their GitHub portfolio, and strengthen their resumes.

The goal of this repository is simple:

- Help beginners decide what to build next.
    
- Provide project ideas with guided resources.
    
- Encourage learning by building.

This repository is for anyone starting projects and getting familiar with the workflow.
To help this list tune in with [[How to Contribute]].


## Projects
---
Projects are divided into three tiers based on the knowledge and experience required to complete them.

🟢 Beginner
🟡 Intermediate
🔴 Advanced

Helping students decide what to do next.

---

### Web Dev

| Link                                                                                                                                        | Description                        | TechStack             | Difficulty |
| ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | --------------------- | ---------- |
| [Chess Game](https://youtu.be/SS5amIbX0Mo?si=rAQPiXXRgPRptbwo)                                                                              | A Chess Game in browser            | javascript, html, css | 🟡         |
| [Websites](https://youtu.be/Rz-rey4Q1bw?si=bGpuRZ4JaykC4E6X)                                                                                | Building 10 Websites               | javascript, html, css | 🟡         |
| [30-30-30](https://www.youtube.com/playlist?list=PLjwm_8O3suyOgDS_Z8AWbbq3zpCmR-WE9)                                                        | 30 Days, 30 Projects, 30 Tutorials | Javascript            | 🟢         |
| [Todo List](https://github.com/dwyl/javascript-todo-list-tutorial)                                                                          | A simple Todo List                 | Javascript            | 🟢         |
| [Loading Screen](https://www.freecodecamp.org/news/how-to-build-a-delightful-loading-screen-in-5-minutes-847991da509f)                      | A simple Loading Screen            | Javascript            | 🟢         |
| [Snake Game](https://www.freecodecamp.org/news/think-like-a-programmer-how-to-build-snake-using-only-javascript-html-and-css-7b1479c3339e/) | A snake game on browser            | HTML, CSS             | 🟢         |


### Python


| **Link**                                                                                      | **Description**                      | **TechStack**      | Difficulty |
| --------------------------------------------------------------------------------------------- | ------------------------------------ | ------------------ | ---------- |
| [PDF Audio](https://www.youtube.com/watch?v=Flm2YHEFd5A)                                      | Convert a PDF to audio               | Python             | 🟢         |
| [Face Recognition](https://www.youtube.com/watch?v=sz25xxF_AVE)                               | Face Recognition + Attendance System | OpenCV             | 🟡         |
| [File Organisation](https://www.youtube.com/playlist?list=PLPhE4fjrl6dlsHysF_FYmq8iFNZZ8EPVi) | File Organisation                    | Python, os, shutil | 🟢         |

---
## A Small Note

I created this repository because I often found myself asking, "What should I build next?"

If you're here, there's a good chance you're in a similar position.

I'm not an expert, just another student trying to learn, improve, and build my first projects. This repository is exactly for that. It will change, expand, and hopefully become better over time as both I and you the reader continue to grow.

Everyone starts somewhere.